...
% Student Name: Yinjiao Zhong

% Student Unique Name: yinjiao
...
def make_move(player, board):
    ...
    '''
    while True:
            #setting inputting and prompting
            move_input=input("" + player_name(player) + "'s move to: ")
            #notice: the input number is a single char and between 1-9
            if not move_input.isdigit() or not '1' <= move_input <= '9':
                print("please input a VALID number between 1-9")
            # Validation #2: the cell is not occupied
            elif board[int(move_input) - 1] != 0:
                print("UNVALID MOVE. Please choose an unoccupied position")
            else:
                #setting the board according to the current_player
...
def check_win_horizontal(board):
    '''Check horizontally, if there is a winner.
            ...
            -------
            int
                The winner's player_id, or 0 if no winner.
            '''
...
def check_win_vertical(board):
    '''Check in any vertical column, if there is a winner.
...
display_board(board)
if moves_left == 0:
    print("Game over! Noboby wins!")
else:
    print("Congratulation! "+ player_name(winner) + " is winner!")
